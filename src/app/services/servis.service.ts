...
export class ServisService {
  ...
  getDrzavu(id: number) {
    return this.httpKlijent.get<Drzava>(
      this.drzaveApi + 'GetCountryById/' + id
    );
    //const drzava = DRZAVE.find(x=>x.countryId === id)!;
    //return of(drzava);
  }
  deleteDrzavaById(id: number) {
    return this.httpKlijent.delete(
  ...
  deleteTeamById(id: number) {
    return this.httpKlijent.delete(
      this.timoviApi + 'DeleteTeamById/' + id,
      this.httpOptions
    );
  }
  updateTim(tim: Tim): Observable<any> {
    return this.httpKlijent.put(
      this.timoviApi + 'UpdateTeam/' + tim.teamId,
      tim,
      this.httpOptions
    );
  }
